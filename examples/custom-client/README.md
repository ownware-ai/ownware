...
HTTP+SSE wire contract** — the same path every application, UI, worker, or optional adapter takes.
...
- **`chat.mjs`** is the reference client: `GET /api/v1/provider-hub/models?scope=connected` → `POST /api/v1/run` → tail the
