...
The runtime Zod schema lives at `packages/cortex/profiles/ownware-design/helpers/manifest.schema.ts`. **That schema is the enforcement boundary.** This README documents the shape humans care about; the Zod schema is what fails the load if a manifest drifts.
...
interface DesignSystemManifest {
  ...
  category: string                    // lowercase-kebab label (open vocabulary)
  surface: 'web' | 'mobile' | 'print' | 'deck'   // default 'web'
  summary: string                     // ≤ 280 chars, 1–2 lines for marketplace cards
...
- **`category`** — primary placement on the marketplace grid. A free lowercase-kebab label (`minimal`, `marketing`, `editorial`, `premium`, `futuristic`, …) — pick whatever vibe word fits the system; it is NOT a fixed enum. If your entry could fit two, pick the dominant one.
...
- **`summary`** — one or two lines, ≤ 280 characters, lead with the feel + the canonical use case. "Clean, single-accent, B2B credibility" is better than "A nice clean design system for businesses."
