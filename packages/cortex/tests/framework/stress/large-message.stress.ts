...
describe('Stress: Large message', () => {
  ...
  it('10KB message content stored without truncation', async () => {
    const t = await gw.state.createThread('mini', 'large-msg')
    ...
    const messages = await gw.state.getMessages(t.id)
    expect(messages.length).toBe(1)
    expect(messages[0]!.content.length).toBe(10_000)
  })

  it('lastMessagePreview is truncated to 200 chars', async () => {
    const t = await gw.state.createThread('mini', 'large-preview')
  ...
  it('100 messages each 1KB on same thread accumulate correctly', async () => {
    const t = await gw.state.createThread('mini', 'many-large')
    for (let i = 0; i < 100; i++) {
      await gw.state.addMessage(t.id, {
        id: `msg_many_${i}`,
        role: 'user',
