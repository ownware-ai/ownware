---
name: debug-agent
description: Systematic evidence-based debugging using runtime NDJSON logs. Generates hypotheses, instruments code, reproduces, analyzes log evidence, and iterates until root cause is proven with cited log lines. Use when the user reports a bug or unexpected behavior.
trigger: /debug-agent
allowedTools:
  - shell_execute
  - readFile
...
3. **Reproduce the bug.**
   ...
   - **If reproduction is straightforward** (a single CLI command, a curl request, a simple script): write and run an ad hoc reproduction script yourself. Tailor it to the runtime — Playwright/Puppeteer for browser bugs, a Node/Python/shell script for backend bugs, etc.
...
- After the user runs the program and confirms completion in their interface, do NOT ask them to type "done"; use the `readFile` tool to read the file at the **log path**.
...
1. Use `grep` to search all files for `#region debug log` markers
