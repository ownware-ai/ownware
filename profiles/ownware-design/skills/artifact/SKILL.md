---
name: artifact
description: The structural blueprint for writing the canonical HTML artifact. Use after discovery is locked in. Defines the exact file shape (doctype, :root tokens, component CSS using var(), data-od-id anchors), the surgical-edit discipline (Edit between data-od-id tags, never rewrite the whole file), and the artifact handoff convention.
trigger: /artifact
---
...
3. **`data-od-id="…"` on every top-level region of the body.** Hero, sections, cards-grid, sidebar, topbar, KPI row, panel-row, footer, every slide. Without these anchors, edits cascade across the whole file.
...
<body>
  <!-- Every top-level region carries data-od-id="…" -->
  <header data-od-id="topnav">…</header>
  <section data-od-id="hero">…</section>
  <section data-od-id="features">…</section>
  <section data-od-id="proof">…</section>
  <section data-od-id="pricing-cta">…</section>
  <footer data-od-id="footer">…</footer>
...
For repeated groups, name the *group*, not the items: `data-od-id="pricing-cards"` on the wrapper, not on each card. Then each card inside can have a sub-id if it earns one (`data-card="starter"`, `data-card="pro"`).

For decks, slides are numbered with their role: `data-od-id="slide-1-cover"`, `data-od-id="slide-2-problem"`. See `deck` skill.
...
Edit between `<div class="panel" data-od-id="chart-panel">` and its closing `</div>`. Leave every other region untouched.
...
If the user asks for a new section ("add a FAQ between proof and CTA"), Edit the body to splice in the new region between the right anchors. Add component CSS for the new region above the responsive block. Add `data-od-id="faq"` to the new wrapper.
...
## The artifact handoff (`<artifact>` block)

When this turn writes a *new* canonical HTML file (or rewrites it), end the response with:
...
- The body of the artifact is a **complete standalone HTML document.** No partial fragments. No summaries. No bash output.
- After `</artifact>`, **stop.** No narrating. No markdown code fence wrapping it.
- The identifier is kebab-case and descriptive — `pricing-page`, `seed-pitch-deck`, `admin-dashboard`.
- The title is the human-readable version — "Pricing page · Acme", "Seed-round pitch deck", "Admin dashboard — overview".

**Skip the artifact block when this turn was only edits to an existing file.** The preview already updated; re-emitting pollutes the file panel. Just reply: which file changed, what changed in one or two lines, what you'd suggest next.

When in doubt, skip the artifact block. Re-emitting an unchanged artifact wastes screen; emitting an empty-shell artifact misleads.
